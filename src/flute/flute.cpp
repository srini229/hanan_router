...
// The lookup tables are compiled into the binary (see flute_lut.cpp), so the
// router needs no data files at runtime. Passing a null path here selects the
// embedded copy; a real path still reads from disk, which keeps the original
// behaviour available for swapping in a different table.
static FILE *flute_open_lut(const char *path, const char *mem, unsigned long len)
{
  if (path != NULL) return fopen(path, "r");
  return fmemopen((void *)mem, (size_t)len, "r");
}
...
FluteState * flute_init(  const char *wirelength_vector_file, const char *routing_tree_file )
{
    ...
    fpwv=flute_open_lut( wirelength_vector_file, flute_powv_lut, flute_powv_lut_len);
    if (!(fpwv)) {
        printf("Error opening FLUTE wirelength table\n") ;
        return(NULL) ;
    }

#if FLUTE_ROUTING==1
    fprt=flute_open_lut( routing_tree_file, flute_port_lut, flute_port_lut_len);
    if (!(fprt)) {
        printf("Error opening FLUTE routing table\n") ;
        return(NULL) ;
    }
#endif
