...
class Module {
  ...
  public:
    ...
    // Authoritative per-hierarchy routing result, from the final routed state.
    // Leaf cells have no nets to route and are skipped.
...
class Netlist {
  ...
  public:
    ...
    int totalUnrouted() const
    {
      if (!_valid) return 0;
      int t = 0;
      for (auto& m : _modules) t += m.second->numUnrouted();
      return t;
    }
    
    void printRouteSummaries() const
    {
      if (!_valid) return;
