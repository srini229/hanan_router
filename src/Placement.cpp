...
void Module::route(Router::Router& router, const std::string& outdir)
{
  ...
  if (!_routed) {
    ...
    // count nets left unrouted after a full attempt (excluded nets don't count).
    ...
    // Rather than guess by reshuffling, drive the order by the conflict itself --
    // promote the nets that were left unrouted so they route ahead of the nets
    // that blocked them. A net's priority grows each time it is left open, so a
    // net that stays blocked keeps climbing until it routes first; ties keep the
    // original (HPWL) order. Iterate (rip-up and reorder), keeping the order that
    // leaves the fewest nets open.
    const int passes = router.reorderPasses();
    const size_t pinned = _routeorder.size();
    // only worth reordering if the free (un-pinned) tail has >1 net to move.
