...
struct Cand {
  int layer;
  Geom::Rect fp;   // footprint reserved on 'layer'
  int var{0};      // sat variable
};
...
bool hitsObstacle(const Geom::Rect& r, const int layer, const Geom::LayerRects& obs)
{
  ...
  for (const auto& o : it->second) if (r.overlaps(o, true)) return true;
  return false;
}
  ...
  for (size_t pi = 0; pi < pins.size(); ++pi) {
    ...
    for (const auto& l : p.shapes) {
      ...
      for (const auto& r : l.second) {
        ...
        if (L + 1 <= lm.maxLayer && lm.canUp(L) && !hitsObstacle(r, L + 1, blockers))
          cands.push_back({L + 1, r, 0});
        if (L - 1 >= lm.minLayer && lm.canDown(L) && !hitsObstacle(r, L - 1, blockers))
          cands.push_back({L - 1, r, 0});
        ...
        for (const auto& st : stubs)
          if (!hitsObstacle(st, L, blockers)) cands.push_back({L, st, 0});
      }
    }
