        ...
        if (it == l.end()) {
          ...
          if (it != l.end() && it->is_object()) {
            try {
              ...
              CERR << "invalid UnitR field for layer " << name << ": " << e.what() << std::endl;
            }
          }
          it = l.find("Pitch");
          ...
          if (mlayer != nullptr) {
            ...
            it = l.find("MaxL");
            if (it != l.end() && it->is_number()) mlayer->setMaxL(static_cast<int>(*it) * uu);
            it = l.find("EndToEnd");
            if (it != l.end() && it->is_number()) mlayer->setE2E(static_cast<int>(*it) * uu);
            it = l.find("Offset");
            if (it != l.end() && it->is_number()) mlayer->setOffset(static_cast<int>(*it) * uu);
            it = l.find("Direction");
          ...
          if (layer1 != nullptr || layer2 != nullptr) {
            ...
            if (it != l.end() && it->is_object()) {
              try {
                ...
                CERR << "invalid R field for via layer " << name << ": " << e.what() << std::endl;
              }
            }
            ...
            if (it != l.end()) {
              ...
              if (itv != it->end() && *itv == "ViaArrayGenerator") {
                ...
                itv = it->find("WidthX");
                if (itv != it->end() && itv->is_number()) wx = static_cast<int>(*itv) * uu;
                itv = it->find("WidthY");
                if (itv != it->end() && itv->is_number()) wy = static_cast<int>(*itv) * uu;
                itv = it->find("SpaceX");
                if (itv != it->end() && itv->is_number()) sx = static_cast<int>(*itv) * uu;
                itv = it->find("SpaceY");
                if (itv != it->end() && itv->is_number()) sy = static_cast<int>(*itv) * uu;
                itv = it->find("NumX");
              ...
              if (itv != it->end() && *itv == "ViaArrayGenerators") {
                ...
                if (itv != it->end()) {
                  for (auto& itvg : *(itv)) {
                    ...
                    if (itva != itvg.end() && itva->is_number()) wx = static_cast<int>(*itva) * uu;
                    itva = itvg.find("WidthY");
                    if (itva != itvg.end() && itva->is_number()) wy = static_cast<int>(*itva) * uu;
                    itva = itvg.find("SpaceX");
                    if (itva != itvg.end() && itva->is_number()) sx = static_cast<int>(*itva) * uu;
                    itva = itvg.find("SpaceY");
                    if (itva != itvg.end() && itva->is_number()) sy = static_cast<int>(*itva) * uu;
                    itva = itvg.find("NumX");
                    if (itva != itvg.end() && itva->is_number()) nx = static_cast<int>(*itva);
                    itva = itvg.find("NumY");
                    if (itva != itvg.end() && itva->is_number()) ny = static_cast<int>(*itva);
                    auto& va = vlayer->addViaArray(wx, wy, sx, sy, nx, ny);
                    auto itve = itvg.find("VencX_L");
                    x = (itve != itvg.end() && itve->is_number()) ? static_cast<int>(*itve) * uu : vlayer->coverlx();
                    itve = itvg.find("VencY_L");
                    y = (itve != itvg.end() && itve->is_number()) ? static_cast<int>(*itve) * uu : vlayer->coverly();
                    va._coverl[0] = x;
                    va._coverl[1] = y;
                    itve = itvg.find("VencX_H");
                    x = (itve != itvg.end() && itve->is_number()) ? static_cast<int>(*itve) * uu : vlayer->coverux();
                    itve = itvg.find("VencY_H");
                    y = (itve != itvg.end() && itve->is_number()) ? static_cast<int>(*itve) * uu : vlayer->coveruy();
                    va._coveru[0] = x;
                    va._coveru[1] = y;
