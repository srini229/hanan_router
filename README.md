...
| `-guidedsym` | no | Route a symmetric pair's second net under the mirror guide only. By default, where the placement mirrored the pair's pins and the first net's pins lie on one side of the axis, the first net is routed clear of every other net's mirror as well, and the second is drawn as its exact mirror; a pair that cannot be mirrored, or whose mirror would not be legal, is guided. |
...
* **Symmetric nets.** Net pairs marked `symmetric_nets` are routed mirror-symmetric. Where the placement mirrored the pair's pins and the first net's pins lie on one side of the axis, the first net is routed clear of every other net's mirror too and the second is drawn as its exact mirror. Otherwise (or with `-guidedsym`) the first net's route, mirrored across the axis, is a soft *deviation-cost* guide for the second net's A\* search (see [Symmetric nets](#symmetric-nets)).
